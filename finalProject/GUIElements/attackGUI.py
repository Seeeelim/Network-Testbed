...
class AttackDialog(QDialog):
    def __init__(self, nodes, ditg_server, parent=None):
        ...
        self.attacker_candidates = []

        for host in self.host_names:
            if host != self.ditg_server:
                self.attacker_candidates.append(host)
        ...
        self.attacker_combo.addItems(self.attacker_candidates)
        self.attacker_combo.currentIndexChanged.connect(self.update_target)
    ...
    def update_target(self):
        ...
        if self.attack_type == "flood":
            self.target_combo.addItem(self.ditg_server)
            ...
            for host in self.host_names:
                if host != attacker and host != self.ditg_server:
    ...
    def pick_script(self):
        ...
        if path:
            ...
            self.attack_type = self.detect_attack_type(path)
    ...
    def validate(self):
        if self.attack_script is None:
            self.launch_btn.setEnabled(False)
            return
        if self.attack_type == "arp" and self.target_combo.currentText() == "":
            self.launch_btn.setEnabled(False)
            return
