#!/usr/bin/env python
"""
ARP Spoofing Attack on the Rural LAN of the Complex Topology.
Victim: rH1 (10.10.10.2)
Normal user: rH2 (10.10.10.3)
Attacker: rH3 (10.10.10.4)
After the attack, D-ITG metrics are printed and the Mininet CLI starts.
"""
...
def run_udp_flood(net):
    victim = net.get('rH1')
    attacker = net.get('rH3')

    print("--- UDP Flood Attack ---")
    victim.cmd('/usr/bin/ITGRecv &')
    # Attacker floods victim with UDP packets
    attacker.cmd('/usr/bin/ITGSend -a {} -T UDP -C 5000 -c 1024 -t 10000 -l flood.dat &'.format(victim.IP()))
    print("[*] UDP flood started from rH3 to rH1")

    time.sleep(12)
    victim.cmd('killall ITGRecv')
    attacker.cmd('killall ITGSend')

    report = attacker.cmd('/usr/bin/ITGDec flood.dat 2>&1')
    attacker.cmd('rm -f flood.dat')
    print(report)
